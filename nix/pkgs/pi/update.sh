#!/usr/bin/env bash
# Refresh pi to the latest GitHub release: fetch the newest tag, prefetch every
# per-platform tarball, and rewrite sources.json (version + all hashes).
#
# Usage:
#   ./update.sh                 # bump to the latest release
#   ./update.sh 0.84.1          # pin an explicit version
#   nix run .#pi.updateScript
set -euo pipefail
