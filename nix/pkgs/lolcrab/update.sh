#!/usr/bin/env bash
# Refresh lolcrab to the latest GitHub release: fetch the newest tag, prefetch
# the source tarball and cargo dependencies, and rewrite sources.json.
#
# Usage:
#   ./update.sh                 # bump to the latest release
#   ./update.sh 0.4.1           # pin an explicit version
#   nix run .#lolcrab.updateScript
set -euo pipefail
...
if [ "$version" = "$current" ]; then
  echo "lolcrab: already at $version" >&2
  exit 0
fi
echo "lolcrab: $current -> $version" >&2

# Prefetch source tarball.
url="https://github.com/$owner/$repo/archive/refs/tags/v$version.tar.gz"
echo "  prefetch source" >&2
hash="$(nix store prefetch-file --json --name "lolcrab-$version.tar.gz" "$url" | jq -r .hash)"
...
echo "lolcrab: wrote $sources_json" >&2
