...
m64="$(echo -e "$m" | base64)"
echo "$m64" > /tmp/test2
tracking_url=$(/home/yigit/.local/share/scripts/gen_tracking_url "$m64" 2> /tmp/testerr)
...
echo "$message" | sed '/^$/q' | grep -q -i 'markdown: true' \
    && msg=$(echo "$message  \n$tracking_url" | cleanHeaders | /home/yigit/.local/share/scripts/convertToHtmlMultipart && echo 1 >> /tmp/state) || msg=$(echo "$message" | cleanHeaders)
echo "$msg" > /tmp/test
